...
class My4DVar:
    ...
    def assimilate(self, HMM, xx, yy, yy_all):
        ...
        if isinstance(self.B, np.ndarray):
            ...
            B = np.cov(xx.T)
            B /= np.max(np.diag(B))
        elif self.B in (None, 'NMC'):
            xx_24 = xx[int(4 * HMM.tseq.dto // HMM.tseq.dt):-int(4 * HMM.tseq.dto // HMM.tseq.dt):int(8 * HMM.tseq.dto // HMM.tseq.dt)]
            xx_48 = xx[int(8 * HMM.tseq.dto // HMM.tseq.dt)::int(8 * HMM.tseq.dto // HMM.tseq.dt)]
            xx_dif = xx_48 - xx_24
            B = np.cov(xx_dif.T)
            B /= np.max(np.diag(B))
        elif self.B == 'eye':
