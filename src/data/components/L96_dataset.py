...
class L96_Dataset(Dataset):
    def __init__(self, 
        ...
        if self.train:
            if self.pred_len > 0:
                self.gt = gt.sel(trial=0)['x'].values[:-self.pred_len, :].astype(np.float32)
                self.xb = xb.sel(trial=0)['x'].values[:-self.pred_len, :].astype(np.float32)
            ...
            self.obs_all = obs_all.sel(trial=0)['x'].values.reshape(self.xa.shape[0], -1, int(dim*obs_partial)).astype(np.float32)
            self.obs_idx = obs.sel(trial=0)['grid'].values.astype(np.int32)
        else:
            if self.pred_len > 0:
                self.gt = gt.sel(trial=1)['x'].values[:-self.pred_len, :].astype(np.float32)
                self.xb = xb.sel(trial=1)['x'].values[:-self.pred_len, :].astype(np.float32)
            ...
            self.obs_all = obs_all.sel(trial=1)['x'].values.reshape(self.xa.shape[0], -1, int(dim*obs_partial)).astype(np.float32)
    ...
    def __getitem__(self, idx):
        if self.normalize:
            ...
            obs_all = np.ones((self.obs_num, self.dim)).astype(np.float32) * self.mean_obs
            obs_idx = self.obs_idx
            obs_all[:, obs_idx] = self.obs_all[idx, :, :]
            obs_all = (obs_all - self.mean_obs) / self.std_obs
            return gt, xb, xa, obs_all[0:self.obs_num,:], obs_idx, self.mean_xa*np.ones_like(gt), self.std_xa*np.ones_like(gt), self.mean_xb*np.ones_like(gt), self.std_xb*np.ones_like(gt)
        else:
            gt = self.gt[idx:idx + 1, :]
            xb = self.xb[idx:idx + 1, :]
            xa = self.xa[idx:idx + self.pred_len + 1, :]
            obs_all = np.ones((self.obs_num, self.dim)).astype(np.float32) * self.mean_obs
            obs_idx = self.obs_idx
            obs_all[:, obs_idx] = self.obs_all[idx, :, :]
            return gt, xb, xa, obs_all[0:self.obs_num,:], obs_idx, np.zeros_like(gt), np.ones_like(gt)