...
def eval_cycles(res_dir, pretrain_dir, system, years, mlda_name, da_model, pred_len, N_trials, dim, da_method, da_N, da_Inf, da_Bx, da_rot, da_loc_rad, da_xN, obs_partial, normalize, da_Lag=0):
    if normalize:
    ...
    for i in range(10):
        ...
        # xb = np.zeros((xa.shape[0], 21, dim))
        # xb[:,0,:] = xa
        # for i in range(xa.shape[0]):
        #     xb_tmp = xa[i]
        #     for j in range(100):
        #         xb_tmp = HMM.Dyn(xb_tmp, 0, 0.01)
        #         if (j+1) % 5 == 0:
        #             xb[i,(j+1)//5,:] = xb_tmp
        #     # xb[i] = xb_tmp

        # rmse_xb = np.zeros((xa.shape[0], 21))
        # for i in range(21):
        #     rmse_xb[:,i] = np.sqrt(np.mean((xb[:,i,:] - xx_[i:xx_.shape[0]-(20-i)]) ** 2, axis=-1))
        # rmse_xb_mls.append(np.asarray(rmse_xb))
        # print('ML Xb RMSE: ', np.mean(rmse_xb, axis=0))
        ...
        rmse_xb = np.zeros((xa.shape[0], 21))
        for i in range(21):
            rmse_xb[:,i] = np.sqrt(np.mean((xb[:,i,:] - xx[i:xx.shape[0]-(20-i)]) ** 2, axis=-1))
        rmse_xb_mls.append(np.asarray(rmse_xb))
        print(f'{da_method} Xb RMSE: ', np.mean(rmse_xb, axis=0))
    ...
    #     dims=['trail', 'lead_time', 'pred_time'],
    #     coords={
    #         'trail': np.arange(10),
    #         'lead_time': np.arange(0, years * OneYear-21*HMM.tseq.dto, HMM.tseq.dto).astype(np.float32),
    #         'pred_time': np.arange(21).astype(np.float32),
    #     },
    #     name='x'
    # )
    ...
    # # print('ML Xa RMSE mean: ', np.mean(rmse_xa_ml_mean, axis=0), ' std: ', np.std(rmse_xa_ml_mean, axis=0))
    # print('ML Xb RMSE mean: ', np.mean(rmse_xb_ml_mean, axis=0), ' std: ', np.std(rmse_xb_ml_mean, axis=0))
